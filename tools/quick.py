...
def run_card(dirn, name, title_text, paras, bottom_limit, title_icon=None, label_override=None, team=None, team_color=None, extra_titles=(), force_size=None):
    ...
    for k in ('ACTION', 'REACTION', 'ACTION2', 'REACTION2', 'REACTION3'):
        if k in L: labels[k] = L[k]
    t = L['title']
    title = dict(text=title_text, cx=1086, cy=(t[1]+t[3])/2, color=L['title_color'])
    if force_size:
        size = force_size; pp = []; prev = None
        for p in paras:
            p = dict(p, size=p.get('size', size))
            if p.get('cy') is None: p['cy'] = prev
            pp.append(p)
        paras = pp
    else:
